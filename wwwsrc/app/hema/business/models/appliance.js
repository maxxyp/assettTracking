var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
define(["require", "exports", "./applianceSafety", "./dataStateProvider", "./dataState"], function (require, exports, applianceSafety_1, dataStateProvider_1, dataState_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var Appliance = /** @class */ (function (_super) {
        __extends(Appliance, _super);
        function Appliance() {
            var _this = _super.call(this, dataState_1.DataState.dontCare, "appliances") || this;
            _this.safety = new applianceSafety_1.ApplianceSafety();
            return _this;
        }
        return Appliance;
    }(dataStateProvider_1.DataStateProvider));
    exports.Appliance = Appliance;
});

//# sourceMappingURL=appliance.js.map
